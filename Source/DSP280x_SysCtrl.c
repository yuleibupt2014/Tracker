...
{
	EALLOW;
	SysCtrlRegs.WDCR = 0x00Af;//使能DOG，时钟频率OSCCLK/512/64，即1.64ms.所以每0.42s前得喂一次狗
	EDIS;
}
//---------------------------------------------------------------------------
// Example: InitPll:
...
void InitPeripheralClocks(void)//高速时钟定标HSPCLK=SYSCLKOUT/（HISPCP*2）,LSPCLK=SYSCLKOUT/(LOSPCP*2)
{
   EALLOW;
